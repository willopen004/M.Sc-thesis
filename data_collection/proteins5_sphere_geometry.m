...
output_dir = 'E:\Yiftach\OneDrive - Technion\kaiser\my_output_directory_parallel\half_hexagon\V14\';  % Specify the directory where you want to save the results
...
result.error_message = '';  % keep for diagnostics
    ...
    fname_template = fullfile(template_dir, '5_protein_configuration_curvature_template_3.fe');
...
function min_Dist = calculate_min_dist(rad_P1, AVG_CURVATURE, NUMBER_OF_PROTEINS)
    ...
    tol          = 1e-6;          % ≈ 1 nm on a 2 km sphere

    Dist = 1;  step = 1;
    while true
        % ---- coordinates --------------------------------------------
        Ax = 0;  Ay = 0;  Az = BIG_SPHERE_RADIUS + ZC;

        Bx = BIG_SPHERE_RADIUS*sin(Dist/BIG_SPHERE_RADIUS)*cos(ang);
        By = BIG_SPHERE_RADIUS*sin(Dist/BIG_SPHERE_RADIUS)*sin(ang);
        Bz = BIG_SPHERE_RADIUS*cos(Dist/BIG_SPHERE_RADIUS) + ZC;

        Cx = BIG_SPHERE_RADIUS*sin(Dist/BIG_SPHERE_RADIUS);
        Cy = 0;
        Cz = BIG_SPHERE_RADIUS*cos(Dist/BIG_SPHERE_RADIUS) + ZC;
        ...
        % ---- stop as soon as both arcs are long enough --------------
        if (dAB >= min_required - tol) && (dBC >= min_required - tol)
            min_Dist = Dist;
            break
        end
        Dist = Dist + step;
...
angle_D = ang(vertices.D, vertices.D_C, vertices.D_E);
angle_E = ang(vertices.E, vertices.E_D, vertices.E_A);
...
Atri = calculate_spherical_triangle_area( ...
    vertices.A(1), vertices.A(2), vertices.A(3), ...
    vertices.B(1), vertices.B(2), vertices.B(3), ...
    vertices.C(1), vertices.C(2), vertices.C(3), BIG_SPHERE_RADIUS_var);

Ctri = calculate_spherical_triangle_area( ...
    vertices.A(1), vertices.A(2), vertices.A(3), ...
    vertices.C(1), vertices.C(2), vertices.C(3), ...
    vertices.D(1), vertices.D(2), vertices.D(3), BIG_SPHERE_RADIUS_var);

Dtri = calculate_spherical_triangle_area( ...
    vertices.A(1), vertices.A(2), vertices.A(3), ...
    vertices.D(1), vertices.D(2), vertices.D(3), ...
    vertices.E(1), vertices.E(2), vertices.E(3), BIG_SPHERE_RADIUS_var);

membrane_area = Atri + Ctri + Dtri;
...
% GET_MODEL_VERTICES
% Builds the geometry to mirror the Surface Evolver parameter block you posted:
% - Five caps A..E on a big sphere centered at (0,0,ZC) with ZC = -R.
% - Bases (contact circles) for each cap.
% - Cap tops along the local outward direction by Hcap.
% - Triangle centers TABC, TACD, TADE (renormalized to big sphere).
% - Periphery points built by BaseP + (rad_P/|target-BaseP|)*(target-BaseP), then renormalized.
...
% Protein-average curvatures (match your SE block choices)
Average_curvature_P_general = 0.15;
...
% ---------------- big-sphere helpers ----------------
R  = BIG_SPHERE_RADIUS;
sa = deg2rad(spin_angle_deg);
s  = Dist / R;                           % central angle
cs = cos(s);  ss = sin(s);

% azimuths as in your SE: B @ 3*sa, C @ 2*sa, D @ 1*sa, E @ 0
azE = 0;
azD = 1*sa;
azC = 2*sa;
azB = 3*sa;

% point on sphere of radius R centered at (0,0,ZC) at (theta=az, polar= s)
% (we keep them in XY plane rotated by az, z by cs)
sphere_pt = @(az) [ R*ss*cos(az), R*ss*sin(az), R*cs + ZC ];

% ---------------- caps A..E on the big sphere ----------------
A = [0, 0, R + ZC];
B = sphere_pt(azB);
C = sphere_pt(azC);
D = sphere_pt(azD);
...
% Plane_offset = R * sqrt(1 - (rad/R)^2). For A: scale only along (ZC->A) line.
PlaneOffset = @(rad) R*sqrt(1 - (rad/R)^2);
BaseScale   = @(rad) PlaneOffset(rad)/R;

% A base: stays on the axis from ZC to A
BaseA = [A(1), A(2), ZC + (A(3)-ZC)*BaseScale(rad_P1)];

% Other bases: scale full vector from center (0,0,ZC)
scale_to_base = @(P,rad) [P(1), P(2), ZC + (P(3)-ZC)] * 0 + ...
                        [P(1)*BaseScale(rad), P(2)*BaseScale(rad), ZC + (P(3)-ZC)*BaseScale(rad)];
BaseB = scale_to_base(B,rad_P2);
BaseC = scale_to_base(C,rad_P3);
BaseD = scale_to_base(D,rad_P4);
BaseE = scale_to_base(E,rad_P5);

% ---------------- unit directions from base toward outer normal ----------------
unit_dir = @(Base) (Base - [0,0,ZC]) / norm(Base - [0,0,ZC]);
...
CtrE = BaseE - dirE*(Rcap_P5 - Hcap_P5);

% ---------------- triangle centers (renormalized to big sphere) ----------------
renorm = @(P) (R / norm(P - [0,0,ZC])) * (P - [0,0,ZC]) + [0,0,ZC];

TABC = renorm( (A + B + C)/3 );
TACD = renorm( (A + C + D)/3 );
TADE = renorm( (A + D + E)/3 );

% ---------------- periphery builder: BaseP -> Target by rad_P, then renorm ----------------
periph = @(BaseP, Target, rad_P) renorm( BaseP + (rad_P / norm(Target - BaseP)) * (Target - BaseP) );
...
% B periphery
v.B_A = B_A; v.B_C = B_C; v.B_TABC = B_TABC;

% C periphery
v.C_B = C_B; v.C_A = C_A; v.C_TABC = C_TABC; v.C_D = C_D; v.C_TACD = C_TACD;

% D periphery
v.D_C = D_C; v.D_A = D_A; v.D_TACD = D_TACD; v.D_E = D_E; v.D_TADE = D_TADE;

% E periphery
v.E_D = E_D; v.E_A = E_A; v.E_TADE = E_TADE;

% Bookkeeping
v.R  = R; v.ZC = ZC; v.spin_angle_deg = spin_angle_deg; v.NUMBER_OF_PROTEINS = NUMBER_OF_PROTEINS;
