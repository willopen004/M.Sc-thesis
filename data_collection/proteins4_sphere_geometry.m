...
output_dir = 'E:\Yiftach\OneDrive - Technion\kaiser\my_output_directory_parallel\half_square\V1\';  % Specify the directory where you want to save the results
...
result.error_message = '';  % keep for diagnostics
    ...
    fname_template = fullfile(template_dir, '4_protein_square_config_curvature_template.fe');
...
% CALCULATE_MIN_DIST
% Finds the smallest Dist such that the relevant spherical separations
% in the CURRENT A-B-C-D geometry are all at least 5*rad_P1.
%
% Current SE geometry:
%   A = (0,0,0)
%   B = azimuth 0
...
%   D = azimuth pi
%
% The active patch edges are:
%   AB, BC, CD
%
% NUMBER_OF_PROTEINS is kept in the signature for compatibility,
% but for the current SE block this function uses the fixed A-B-C-D layout.
    ...
    while true
        ...
        % C : azimuth pi/2
        Cx = BIG_SPHERE_RADIUS * sinus * cos(pi/2);
        Cy = BIG_SPHERE_RADIUS * sinus * sin(pi/2);
        Cz = z_ring;

        % D : azimuth pi
        Dx = BIG_SPHERE_RADIUS * sinus * cos(pi);
        Dy = BIG_SPHERE_RADIUS * sinus * sin(pi);
        Dz = z_ring;

        % ============================================================
        % ANCHOR: spherical distances relevant to current patch
        % ============================================================
        ...
        dCD = calculate_spherical_distance(Cx, Cy, Cz, Dx, Dy, Dz, BIG_SPHERE_RADIUS);

        % Stop once all relevant neighbor spacings are large enough
        if (dAB >= min_required - tol) && ...
           (dBC >= min_required - tol) && ...
           (dCD >= min_required - tol)
    ...
    fprintf('min Dist for current A-B-C-D geometry is: %.3f\n', min_Dist);
end
...
% CALCULATE_PSI for the current four-protein geometry A..D on the big sphere.
%
% Sector definitions:
...
%   PHI           : total base coverage / membrane patch area
%   membrane_area : area(ABC) + area(ACD)
...
% A is now sum of two wedges only: AB-AC and AC-AD
...
angle_D = ang(vertices.D, vertices.D_C, vertices.D_A);
...
totalBase = area_A + area_B + area_C + area_D;

% --------- PSI ----------
% Kept exactly in the same style as your previous code
if RATIO_VAR == 1
    PSI = 1;
elseif RATIO_VAR == 11
    PSI = 0.5;
...
% --------- membrane patch area = △ABC + △ACD ----------
area_ABC = calculate_spherical_triangle_area( ...
...
area_ACD = calculate_spherical_triangle_area( ...
    vertices.A(1), vertices.A(2), vertices.A(3), ...
    vertices.C(1), vertices.C(2), vertices.C(3), ...
    vertices.D(1), vertices.D(2), vertices.D(3), ...
    BIG_SPHERE_RADIUS_var);

membrane_area = area_ABC + area_ACD;
...
% CALCULATE_AREA for the current four-protein geometry A..D.
%
% Current membrane patch:
%   - triangle ABC
%   - triangle ACD
%
% Therefore:
...
% - Four caps A, B, C, D on a big sphere centered at (0,0,ZC), with ZC = -R
% - A is the top point at (0,0,0)
% - B, C, D are placed exactly as in the SE code:
%       B: azimuth 0
%       C: azimuth pi/2
%       D: azimuth pi
% - Bases (contact circles), cap tops, and small-sphere centers
% - Triangle centers TABC and TACD, renormalized to the big sphere
% - Periphery points exactly following the SE construction:
%       BaseP + (rad_P / |Target-BaseP|)*(Target-BaseP), then renormalized
%
% IMPORTANT:
% This function now matches the CURRENT SE block you posted,
% which contains only A-D. The old E/TADE geometry was removed.
...
spin_angle         = deg2rad(spin_angle_deg); %#ok<NASGU> % kept for bookkeeping

AVG_CURVATURE      = AVG_CURVATURE_var;
BIG_SPHERE_RADIUS  = 2 / AVG_CURVATURE;
ZC                 = -BIG_SPHERE_RADIUS;

% ---------------- protein curvature block ----------------
% Matches your current SE block:
...
% ---------------- cap positions A..D ----------------
% Exact match to SE block
A = [0, 0, 0];

B = [BIG_SPHERE_RADIUS * sinus * cos(0), ...
     BIG_SPHERE_RADIUS * sinus * sin(0), ...
     z_ring];

C = [BIG_SPHERE_RADIUS * sinus * cos(pi/2), ...
     BIG_SPHERE_RADIUS * sinus * sin(pi/2), ...
     z_ring];

D = [BIG_SPHERE_RADIUS * sinus * cos(pi), ...
     BIG_SPHERE_RADIUS * sinus * sin(pi), ...
     z_ring];
...
% ---------------- renormalization to big sphere ----------------
renorm = @(P) (R / norm(P - sphere_center)) * P;
